...
if my_rank == electionCaller:
    ...
    for request in reqList: # If there was a response, I am not the leader
        ...
        if leaderCandidate > leader and ack[0] != False:
            print(ack[1])
            leader = leaderCandidate # Choose the leader

    print("-----------------------------------------------------------------------")
    print("PROCESS " + str(my_rank) + " IN ITS ROLE OF ELECTION ORGANISER, DECLARES " + str(leader) + " AS THE WINNER!!!\n")

    leader = comm.bcast(leader, root=my_rank)
    ...
    while(True):
        ...
        if(waitResult):
            leader = comm.bcast(leader, root=electionOrganizerID) # Election leader will be announced through the election organizer broadcast  
            print("PROCESS " + str(my_rank) + " RECOGNIZES PROCESS " + str(leader) + " AS ITS LEADER!!!\n")